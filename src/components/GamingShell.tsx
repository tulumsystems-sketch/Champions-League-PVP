  ...
  return (
    <div ref={rootRef} className={cn("relative min-h-screen text-white", className)}>
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_0%,rgba(255,22,56,0.16),transparent_38%),radial-gradient(circle_at_92%_8%,rgba(155,0,24,0.22),transparent_34%)]" />
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#ff1638]/80 to-transparent" />
        <div className="arena-scan" />
        {Array.from({ length: 20 }).map((_, index) => (
          <span
            key={index}
            data-spark
            className="absolute rounded-full bg-[#ff1638]/70 will-change-transform"
            style={{
              left: `${(index * 17) % 100}%`,
              top: `${(index * 29) % 100}%`,
              width: index % 3 === 0 ? 3 : 2,
